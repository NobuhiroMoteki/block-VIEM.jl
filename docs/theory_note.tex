...
\begin{itemize}
  ...
  \item orientation: single tilt $(\alpha, \beta, \gamma) = (0,\pi/4,0)$;
  \item discretisations: DDA uses $N_{\text{dip}} = 2236$ dipoles
        ...
        $N_{\text{dof}} = 8746$.
\end{itemize}
The PCAS forward amplitudes agree to about two percent:
\begin{table}[h]
\centering
\caption{Cross-validation of VIEM against block-DDA\_Py on a single
oblate AR = 3 spheroid at $\beta = \pi/4$. The relative error is
$\abs{\Delta S}/\abs{S_{\text{DDA}}}$.}
\label{tab:dda-viem-spheroid}
\begin{tabular}{lcc}
\toprule
Observable          & block-DDA\_Py                 & block-VIEM.jl \\
\midrule
$S_{\text{fw},\theta}$ & $+0.23503 + 0.09351 i$ & $+0.23143 + 0.09164 i$ \\
$S_{\text{fw},\phi}$   & $+0.23836 + 0.18567 i$ & $+0.23480 + 0.17937 i$ \\
$C_{\text{ext}}$       & $\SI{0.17811}{\square\micro\metre}$
                                                & $\SI{0.17291}{\square\micro\metre}$ \\
\midrule
$\abs{\Delta S_{\text{fw},\theta}}/\abs{S_{\text{fw},\theta}^{\text{DDA}}}$
  & \multicolumn{2}{c}{$1.60\%$}\\
$\abs{\Delta S_{\text{fw},\phi}}/\abs{S_{\text{fw},\phi}^{\text{DDA}}}$
  & \multicolumn{2}{c}{$2.39\%$}\\
$\abs{\Delta C_{\text{ext}}}/C_{\text{ext}}^{\text{DDA}}$
  & \multicolumn{2}{c}{$2.92\%$}\\
\bottomrule
\end{tabular}
\end{table}
